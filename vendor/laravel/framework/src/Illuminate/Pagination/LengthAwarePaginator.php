...
class LengthAwarePaginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, Jsonable, LengthAwarePaginatorContract {
	...
	public function __construct($items, $total, $perPage, $currentPage = null, array $options = [])
	{
		...
		$this->pageMin = $currentPage - $perPage;
		if($this->pageMin < 1 ) $this->pageMin = 1;
		$this->pageMax = $currentPage + $perPage;
		if($this->pageMax > ceil($total / $perPage)) $this->pageMax = ceil($total / $perPage);
	}
